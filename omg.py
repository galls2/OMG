...
class OmgModelChecker(object):
    ...
    def _initialize_abstraction(self):
        self._abstract_structure = AbstractStructure(self._kripke, self._trivial_split)
    ...
    def _handle_and(self, node, spec, is_strengthen, left_operand, right_operand):
        return _label_state(
            self._handle_ctl_and_recur(node, left_operand, is_strengthen) and self._handle_ctl_and_recur(node,
                                                                                                         right_operand,
                                                                                                         is_strengthen),
            node, spec)
    ...
    def _handle_or(self, node, spec, is_strengthen, left_operand, right_operand):
        return _label_state(
            self._handle_ctl_and_recur(node, left_operand, is_strengthen) or self._handle_ctl_and_recur(node,
                                                                                                        right_operand,
                                                                                                        is_strengthen),
            node, spec)
    ...
    def _handle_arrow(self, node, spec, is_strengthen, left_operand, right_operand):
        return _label_state(
            (not self._handle_ctl_and_recur(node, left_operand, is_strengthen)) or (
                self._handle_ctl_and_recur(node, right_operand, is_strengthen)),
            node, spec)
    ...
    def _check_inductive_av(self, goal, p, to_visit):
        ...
        while abs_states_lead:
            ...
            if res is True:
                ...
                if not to_close_abstract.get_descriptive_formula().assign_state(src_to_witness).is_sat():
                    print 'upup1'
                    print str(node)

                if not self._find_abstract_classification_for_state(witness_state).get_descriptive_formula().assign_state(witness_state).is_sat():
                    print 'upup2'
                    print str(node)
                ...
                if concretization_result.exists():
                    ...
                    self.c += 1
                    if not any([_to for _to in to_close_nodes
                                if _to.get_abstract_label() == abs_src_witness]):
                        print 'lalala'
                        res = to_close_abstract.get_descriptive_formula().assign_state(src_to_witness).is_sat()
                        print res
                        assert False
                        #print_class_trees(abs_src_witness.get_classification_node())
                    ...
                    logger.debug('Splitting '+abs_src_witness.get_classification_node().get_binary_string() + ' w.r.t. '+ \
                                 self._find_abstract_classification_for_state(witness_state).get_classification_node().get_binary_string())
    ...
    def _handle_proving_trace(self, is_strengthen, node, node_to_explore, spec, to_return):
        if is_strengthen:
            ...
            _map_upward_from_node(node_to_explore,
                                  lambda current_node: current_node.add_label(spec, to_return),
                                  node.get_parent())
        return to_return
    ...
    def _handle_ex(self, node, spec, is_strengthen, operand):
        ...
        for child_node in children_nodes:
            ...
            if res:
                ...
                if is_strengthen:
                    self._refine_split_ex(node, [child_node.concrete_label], check_trivial=True,
                                          known_reclassification=True)
                return True
    ...
    def _refine_split_ex(self, node_src, dst_states, check_trivial, known_reclassification=None):
        ...
        if self._abstract_structure.is_known_E_must_between(node_src.get_abstract_label(), witness_abstract_states):
            return
    ...
    def _refine_split_ax(self, src_node, dst_nodes, check_trivial, known_reclassification=None):
        ...
        witness_abstract_states = unique([dst.get_abstract_label() for dst in dst_nodes])
        if self._abstract_structure.is_known_may_over_between(src_node.get_abstract_label(), witness_abstract_states):
            return
