...
#            logger.debug('AV:: Trying to close abstract state of' + to_close_nodes[0].description() + ' :'+str(goal))
 ...
 #               logger.debug(' Success!')
    ...
    def _refine_split_next(self, src_node, witness_abstract_states, split_state_function, query_getter, check_trivial,
        ...
        new_internal = self._abstraction.split(query, original_classification_leaf, query_labeling_mapper, '#'.join([abst.get_debug_name() for abst in witness_abstract_states]))
